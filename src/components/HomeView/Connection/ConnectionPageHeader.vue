<template>
  ...
  >
    <template #extra>
      ...
      <a-button key="2" @click="handleExport"><ExportOutlined />Export</a-button>
    </template>
...
function handleImport(imported: ConnectionInfoState) {
  try {
    ...
    connInfoStore.$reset()
    message.error('Patching to current state failed, please check your input and try again.')
  }
}
...
const handleFileSelected = async () => {
  ...
  const files = import_file_input.value?.files
  ...
  const import_text = file ? await file.text() : null
  try {
    // if file loaded successfully, parse it as json.
    const parsed = import_text ? JSON.parse(import_text) : null
    console.log(parsed)
    if (parsed) {
    ...
    message.error('Importing ' + filename + ' failed because it is not a valid JSON config file.')
  }
}
// #endregion
...
const test_websocket_status = computed(
  () =>
    ...
    connStatusState.value.websockets.get('websocket-lablog') ?? WebSocketStatus.Unknown,
)
...
const user_agent_short =
  user_agent.length < 50 ? user_agent : user_agent.slice(0, 25) + ' ... ' + user_agent.slice(-25)
