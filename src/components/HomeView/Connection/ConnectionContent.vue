<template>
  ...
  >
    <a-form :model="connInfoState" :label-col="labelCol" :wrapper-col="wrapperCol">
      <a-form-item label="RESTful API Origin">
        <a-input-group compact>
          ...
          <a-input
            ...
            placeholder="blogapi.example.com"
            :disabled="disable_restful"
          />
          <a-input-number
      ...
      <a-form-item label="gRPC API Origin">
        <a-input-group compact>
          ...
          <a-input-number
            ...
            :disabled="disable_grpc"
          />
          <a-button type="primary" style="width: 60px" :disabled="disable_grpc" @click="onTestGrpc"
            >Test</a-button
          >
        </a-input-group>
      </a-form-item>
...
const { connInfoState, restful_endpoint, websocket_endpoint } = storeToRefs(connInfoStore)
...
const onTestRestful = async () => {
  console.log('Testing RESTful Availability with config: ', toRaw(connInfoState.value.restful))
  ...
  try {
    ...
    if (response?.ok) {
      ...
      console.log('Server reported an issue, HTTP Response Code: ', response?.status)
...
const onTestWebsocket = () => {
  ...
  const wsid = 'websocket-taskgraph'
  const target_url = websocket_endpoint.value
  console.log('Testing WebSocket Availability with config: ', toRaw(connInfoState.value.websocket))
  ...
  try {
    ...
    webSocketManagerStore.createNewWebSocket({
      ...
      on_open: function (this, ev) {
        ...
        connStatusStore.connStatusState.websockets.set(wsid, WebSocketStatus.Connected)
        this.close()
      },
      on_close: function (this, ev) {
        console.log('websocket closed', ev)
        if (ev.wasClean) {
          connStatusStore.connStatusState.websockets.set(wsid, WebSocketStatus.Ready)
        } else {
          console.log('Websocket closed but not clean!')
          connStatusStore.connStatusState.websockets.set(wsid, WebSocketStatus.Error)
        }
      },
...
const onTestGrpc = () => {
  console.log('Testing gRPC Availability with config: ', toRaw(connInfoState.value.grpc))
}
